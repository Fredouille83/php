...
(function (manyToMany, xrtc) {
    ...
    // Set middle tier service proxies (on server)
	  // This is the server page which handles the calls 
		...
		connectionCreated: function(connectionData) {
			...
			manyToMany.subscribe( _connection, xrtc.Connection.events );
			...
			_connection
				...
				.on( xrtc.Connection.events.remoteStreamAdded, function (data) {
					...
					manyToMany.addVideo(data);
					manyToMany.refreshRoom();
				})
        ...
        // _textChannel is an array to handle multiple users
          ...
          var remoteUser = data.channel.getRemoteUser().name;
					...
					manyToMany.subscribe(_textChannel[remoteUser], xrtc.DataChannel.events);
					_textChannel[remoteUser].on( xrtc.DataChannel.events.sentMessage, function(msgData) {
						...
						manyToMany.addMessage(_textChannel[remoteUser].getRemoteUser().name, msgData.data);
					});
          
					manyToMany.addMessage("XirSys", "You connected with " + remoteUser + ".");
        ...
        // You may wish to add real functionality here
          ...
          manyToMany.addMessage("XirSys", "You disconnected from " + data.user.name + ".");
      ...
      manyToMany.addMessage(roomInfo.user.name, message, true);
      ...
      if (textChannelSize == 0) {
        manyToMany.addMessage("XirSys", "No one can hear you right now.");
      } else {
    ...
    // Adds a message to local chat box
      ...
      $chat[0].scrollTop = $chat[0].scrollHeight;
		...
		refreshRoom: function() {
			...
			var contacts = manyToMany.convertContacts(_room.getUsers());
			for (var index = 0, len = contacts.length; index < len; index++) {
				manyToMany.addUser(contacts[index]);
			}
		},
    ...
    // Only connects if there isn't already an existing connection with participant
      ...
      if (!_textChannel[participant]) {
        manyToMany.room().connect(participant, { createDataChannel: 'auto' });
      }
    },
